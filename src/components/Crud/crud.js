  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        crud.loading = true
        // 请求数据
        crud.crudMethod.get(crud.getQueryParams()).then(data => {
          const table = crud.getTable()
          if (table && table.lazy) { // 懒加载子节点数据，清掉已加载的数据
            table.store.states.treeData = {}
            table.store.states.lazyTreeNodeMap = {}
          }
          crud.page.total = data.total || 0
          crud.data = data.rows || []
    ...
    doDelete(data) {
      ...
      crud.crudMethod.del(ids).then((response) => {
        if (delAll) {
    ...
    doExport() {
      ...
      crud.crudMethod.download(crud.getQueryParams()).then(result => {
